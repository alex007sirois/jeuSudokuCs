...
namespace Sudoku.Metier
{
    public class Grille
    {
        ...
        internal Grille(List<byte> grilleDepart)
        {
            ...
            for (int i = 0; i < DIMENSION; i++)
            {
                ...
                {
                    chiffre = grilleDepart[DIMENSION*i+j];

                    cases[i, j] = new Case(chiffre == 0 ? true : false, chiffre, DIMENSION);
                }
            }
        }

        internal Grille(List<Tuple<byte,bool>> grilleDepart)
        {
            this.cases = new Case[DIMENSION, DIMENSION];
            ...
            for (int i = 0; i < DIMENSION; i++)
            {
                for (int j = 0; j < DIMENSION; j++)
                {
                    chiffre = grilleDepart[DIMENSION * i + j].Item1;
                    modifiable = grilleDepart[DIMENSION * i + j].Item2;

                    cases[i, j] = new Case(modifiable, chiffre, DIMENSION);
                }
            }
        }
        ...
        {
            List<Tuple<byte, bool>> list=new List<Tuple<byte,bool>>();

            foreach(Case c in cases)
                list.Add(new Tuple<byte, bool>(c.Chiffre, c.Modifable));
